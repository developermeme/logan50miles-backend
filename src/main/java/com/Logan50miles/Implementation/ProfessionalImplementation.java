...
public class ProfessionalImplementation implements ProfessionalService {
    ...
    public ResponseEntity<String> loginPlayer(String emailId, String password) {
        ...
        if(playerData!=null) {
            if(encoder.matches(password, playerData.getPassword())){
                result = ResponseEntity.status(HttpStatus.OK).body("{\"message\" : \"Player Login Success\", \"playerid\" : \""+playerData.getPid()+"\", \"emailId\" : \""+playerData.getEmail()+"\"}");
            }else{
    ...
    public String updatePlayer(Professionals playerdetails, MultipartFile file) throws ResourceNotFoundException, IOException {
    	Professionals player = professionalsRepository.findById(playerdetails.getPid()).orElseThrow(()-> new ResourceNotFoundException("Resource Not found"));
    	if(file!= null){
			this.uploadImage(file,"event");
    ...
    public ResponseEntity<String> forgotPassword(String emailId) throws ParseException {
        ...
        if(playerData!=null) {
        	...
        	mail.sendMail("PIKE50MILES Reset Password", "PIKE50MILES verification OTP is:" + Otp.getOtp(), emailId, "no_reply@memebike.tv", "Sal76928");
    ...
    @Override
    public PlayerProfile updateProfileFeed
    (PlayerProfile pp) throws ResourceNotFoundException {
    	PlayerProfile existance = playerProfileRepository.findById(pp.getPpid()).orElseThrow(()-> new ResourceNotFoundException("Resource Not found"));
    	...
    	existance.setProid(existance.getPid());
    ...
    public String feedLike(int id) throws ResourceNotFoundException {
    	PlayerProfile pp = playerProfileRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("Resource Not found"));
    ...
    public FeedComment updateFeedComment(FeedComment comment) throws ResourceNotFoundException {
    	FeedComment fc = feedCommentRepository.findById(comment.getCid()).orElseThrow(()-> new ResourceNotFoundException("Resource Not found"));
    ...
    public ProfileComment updateProfileComment(FeedComment comment) throws ResourceNotFoundException {
    	ProfileComment fc = profileCommentRepository.findById(comment.getCid()).orElseThrow(()-> new ResourceNotFoundException("Resource Not found"));
    	if(comment.getComment()!=null) {
    		fc.setComment(comment.getComment());
    	}
    	else {
    ...
    public List<Professionals> viewProfessionals(){
    	Events e = eventsRepository.findByEventtype("PROFESSIONAL");
